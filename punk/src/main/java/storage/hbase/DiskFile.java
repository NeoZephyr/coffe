...
public class DiskFile implements Closeable {
    ...
    public static class BlockMeta implements Comparable<BlockMeta> {
        ...
        /**
         * Only used for to seek a target block meta. we only care about the lastKV, so
        ...
        private static BlockMeta createSeekDummy(KeyValue lastKV) {
            return new BlockMeta(lastKV, 0L, 0L, Bytes.EMPTY);
        }
        ...
        public int getSerializeSize() {
            ...
            return lastKV.getSize() + OFFSET_SIZE + SIZE_SIZE + BF_LEN_SIZE + bloomFilter.length;
        }
        ...
        public static BlockMeta parseFrom(byte[] buf, int offset) throws IOException {
            ...
            KeyValue lastKV = KeyValue.fromBytes(buf, offset);
            pos += lastKV.getSize();
    ...
    public static class BlockReader {
        ...
        public static BlockReader parseFrom(byte[] buffer, int offset, int size) throws IOException {
            ...
            for (int i = 0; i < kvSize; i++) {
                KeyValue kv = KeyValue.fromBytes(buffer, offset + pos);
                kvBuf.add(kv);
                crc32.update(buffer, offset + pos, kv.getSize());
                pos += kv.getSize();
            }
    ...
    public static class DiskFileWriter implements Closeable {
        ...
        public void append(KeyValue kv) throws IOException {
            ...
            assert kv.getSize() + BlockWriter.KV_SIZE_LEN + BlockWriter.CHECKSUM_LEN < BLOCK_SIZE_UP_LIMIT;

            if ((currentWriter.getKeyValueCount() > 0)
                    && (kv.getSize() + currentWriter.size() >= BLOCK_SIZE_UP_LIMIT)) {
    ...
    private class InternalScanner implements SeekScanner {
