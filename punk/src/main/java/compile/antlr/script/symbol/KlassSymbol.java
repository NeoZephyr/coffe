...
public class KlassSymbol extends Symbol {

    // 父类
    KlassSymbol parent = null;

    ThisSymbol thisRef = null;

    SuperSymbol superRef = null;

    DefaultCtorSymbol defaultCtorSymbol = null;

    // 最顶层的基类
    private static KlassSymbol rootKlass = new KlassSymbol("Object", null);

    public KlassSymbol(String name, ParserRuleContext ctx) {
        this.name = name;
        this.ctx = ctx;

        thisRef = new ThisSymbol(null, ctx);
        thisRef.type = null;
    }

    public void setParent(KlassSymbol parent) {
        this.parent = parent;

        // superRef 引用的也是自己
        superRef = new SuperSymbol(null, ctx);
        superRef.type = null;
    }
