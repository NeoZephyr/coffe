...
import compile.antlr.script.runtime.AsmGenerator;
import compile.antlr.script.runtime.ByteCodeGenerator;
import compile.antlr.script.runtime.ScriptCompiler;
...
public class ScriptRunner {
    ...
    public static void main(String[] args) {
        ...
        if (script == null) {
            ...
            runJavaClass("ScriptClass", bc);
        } else {
            ScriptCompiler compiler = new ScriptCompiler();
            AnnotatedTree tree = compiler.compile(script, verbose);
    ...
    private static void generateAsm(String script, String outputFile) {
        ...
        AsmGenerator asmGen = new AsmGenerator(at);
    ...
    private static byte[] generateByteCode(String script) {
        ScriptCompiler compiler = new ScriptCompiler();
        AnnotatedTree at = compiler.compile(script);
        ByteCodeGenerator bcGen = new ByteCodeGenerator(at);
        byte[] bc = bcGen.generate();
        String outputFile = "ScriptClass.class";
     ...
     * REPL
     */
    private static void REPL(boolean verbose) {
        ...
        while (true) {
            try {
                ...
                if (line.endsWith(";")) {
                    // 解析整个脚本文件
                    AnnotatedTree tree = compiler.compile(script + scriptLet, verbose);
